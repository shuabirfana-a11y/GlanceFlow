...
- 厂商无关 `CalendarPort`、创建请求、快照、确认、预检、验证和事务记录模型。
- 支持稳定 event_id、范围查询、幂等和故障注入的内存日历。
- 重复检查、主活动冲突分钟数和冲突二次确认。
- 单/双事件原子创建、event_id 回读、关键字段验证和补偿回滚。
- 按事务 event_id 精准撤销、部分失败留痕和重复撤销保护。
- 首个或第二个事件创建后超时的幂等恢复。
- 官方 Google Calendar 客户端契约适配器及私有扩展字段幂等策略。
- `calendar-demo` 四场景演示和 `stage3_results.json`。

### Changed

- 项目版本升级至0.3.0；原 Stage 1/2 接口和67项测试保持兼容。
- 增加官方 Google 客户端依赖和凭据/token忽略规则。

### Fixed during review

- 强制创建请求私有元数据与顶层事务字段一致。
- 供应商预检失败不再留下可确认的 `PLANNED` 事务。
- 确认后计划字段变化会使原确认失效。
- 补全第二事件落库后超时的双事件幂等恢复。
