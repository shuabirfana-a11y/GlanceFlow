...
## ADR-020：公开网络通知与真实校园实拍分层

公开高校网页中的通知使用独立的 `PUBLIC_WEB` 来源类型和 `PW-xxx` 标识，不得称为真实校园实拍。公开可访问不等于允许再分发；没有明确许可时，图片只允许进入 Git 忽略的本机缓存，仓库仅提交来源 URL、权利判断、匿名标注和自动报告。页面文本标注不能替代图片 OCR 证据；在图片缓存和人工视觉隐私复核完成前，正式评测必须保持 `NOT EXECUTED`，不得用 0% 伪装未执行结果。

## ADR-021：PUBLIC_WEB 正式运行前冻结准入集合

公开网络图片必须依次通过来源、下载完整性、去重、逐图隐私、必要脱敏、Scope、Ground Truth 和真实人工复核门禁。Codex 辅助整理不得冒充真人 reviewer；未复核记录使用 `human_review_status=PENDING`。只有通过全部门禁的样本才能写入 `evaluation_set.lock.json`，且正式 OCR/Agent 运行必须读取该锁；锁为空或状态非 `READY` 时禁止正式运行。下载被执行环境阻止时记录失败，不以占位图或网络搜索缩略图替代。
