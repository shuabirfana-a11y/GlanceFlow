...
- `manifest.csv` 只接受 `RD-001` 格式且 `source_type=REAL_CAMPUS_CAPTURE` 的团队实拍样本；46 个固定种子合成样本和 `PUBLIC_WEB` 样本分别保存在各自目录，三者不混算。
...
每行必须同时满足：`source_type=REAL_CAMPUS_CAPTURE`、`permission_confirmed=true`、`privacy_reviewed=true`、`sanitized=true`、`annotation_status=APPROVED`，且图片位于 `evaluation/real_data/sanitized/`。标注者、复核者及 sample_id 必须和 JSON 一致。
